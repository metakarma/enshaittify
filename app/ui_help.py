...
Streamlit help bubbles are plain text only: no Markdown, no LaTeX. Use ASCII-friendly
math (sigmoid, exp, subscripts as words) so formulas read correctly.
"""

from __future__ import annotations

# sigmoid(x) = 1/(1+exp(-x)); we spell this out in tooltips where needed.
...
K_F = (
    ...
    "Math: F(t) = F_max * sigmoid(k_F * (t - t_F_inflection)), where sigmoid(x) = 1/(1+exp(-x)). "
    "F(t) enters Q_open = clip(Q_open_base + mu * F(t) * ln(1 + N_open), 0, 1). "
...
Q_PLAT_BASE = (
    ...
    "Math: Let s_N = sigmoid(k_plat * (N_platform - plat_threshold)). Then "
    "Q_platform = clip(Q_plat_base + (Q_plat_max - Q_plat_base) * s_N - E * enshit_quality_drag, 0, 1). "
    "Q_plat_base is the intercept when s_N is near 0 (and E = 0)."
)
...
ENSHIT_THRESHOLD = (
    ...
    "Let t_star be the first time s_plat >= enshit_threshold. For t >= t_star and s_plat >= enshit_threshold, "
    "time_ramp = min(1, (t - t_star) / enshit_ramp_years); else time_ramp = 0. "
    "E = clip(E_max * sigmoid(k_E * (s_plat - enshit_threshold)) * time_ramp * brake, 0, E_max) with "
    "brake = 1 - 0.5 * min(1, N_open / 0.3). "
    "This slider is enshit_threshold (often written theta in notes)."
)
...
A_MAX = (
    ...
    "Each consumer type i gets utility term epsilon_i * A(t) in U_open (not in U_platform)."
)
...
SCENARIO_PRESET = (
    ...
    "and enshit_threshold. All other parameters (delays, advanced sliders, mu, E_max, etc.) stay "
...
Q_PLAT_MAX = (
    ...
    "sigmoid(k_plat * (N_platform - plat_threshold)) - E * enshit_quality_drag, 0, 1)."
)

K_PLAT = (
    "Steepness of how fast platform quality rises with cumulative platform adopters N_platform.\n\n"
    "Math: same sigmoid(k_plat * (N_platform - plat_threshold)) term inside Q_platform above."
)
...
PLAT_THRESHOLD = (
    ...
    "Math: argument of sigmoid in Q_platform is k_plat * (N_platform - plat_threshold)."
)
...
ENSHIT_QUALITY_DRAG = (
    ...
    "Also U_platform_i includes -delta_i * E separately."
)
...
L_MAX = (
    ...
    "is lost. U_platform_i includes -gamma_i * L."
)
...
E_MAX = (
    ...
    "Math: E = clip(E_max * sigmoid(k_E * (s_plat - enshit_threshold)) * time_ramp * brake, 0, E_max). "
...
K_E = (
    ...
    "Math: sigmoid(k_E * (s_plat - enshit_threshold)) in E above."
)
...
ENSHIT_RAMP_YEARS = (
    ...
    "Math: if eligible, time_ramp = min(1, (t - t_star) / enshit_ramp_years) where t_star is first crossing "
    "time; else 0."
)
...
K_A = (
    ...
    "Math: A(t) = A_max * (1 - exp(-k_A * t)) in U_open via epsilon_i * A(t)."
)

V_BASE = (
    "Baseline utility from values alignment / steward loyalty, before the awareness sigmoid.\n\n"
    "Math: V(t) = V_base + V_awareness * sigmoid(k_V * (t - t_V_inflection)); enters U_open as zeta_i * V(t)."
)

V_AWARENESS = (
    "Extra values premium as public understanding of steward loyalty grows.\n\n"
    "Math: added to V(t) as V_awareness * sigmoid(k_V * (t - t_V_inflection))."
)

K_V = (
    "Steepness of the awareness sigmoid in calendar time.\n\n"
    "Math: inside V(t) = V_base + V_awareness * sigmoid(k_V * (t - t_V_inflection))."
)

T_V_INFLECTION = (
    "Calendar midpoint (years) for the values-awareness logistic.\n\n"
    "Math: argument (t - t_V_inflection) inside sigmoid for V(t)."
)
...
CHOICE_LAMBDA = (
    ...
    "P_open_i = 1 / (1 + exp(-lambda * (U_open_i - U_platform_i))). "
    "U_open_i = alpha_i * Q_open + beta_i * N_open + epsilon_i * A + zeta_i * V; "
    "U_platform_i = alpha_i * Q_platform + beta_i * N_platform - gamma_i * L - delta_i * E."
)
...
CONSUMER_MIX_WEIGHT = (
    ...
    "Bass arrival weights over the horizon."
)
...
CONSUMER_LEAVE_OPEN = (
    ...
    "Math: P(open->platform) = sigmoid(lambda * (U_platform - U_open - leave_open_cost_i)), "
    "sigmoid(x)=1/(1+exp(-x)). Distinct from global lock-in L in U_platform."
)
...
CONSUMER_LEAVE_PLATFORM = (
    ...
    "Math: P(platform->open) = sigmoid(lambda * (U_open - U_platform - leave_platform_cost_i))."
)
