...
**Federated Equilibrium** — **`k_F=0.65`**, **`Q_plat_base=0.44`**, **`A_max=0.55`**, **`enshit_threshold=0.76`**, **`choice_lambda=0.95`**.  
A **middle world** calibrated for **~30% cumulative open TAM** at 8y (clearly above **Platform Capture**): institutions move at a **credible** pace, platforms get a **moderate** head start, **enshittification is delayed** until platforms are **very** dominant among adopters, and **low λ** keeps adoption **split** rather than winner-take-all. The name signals **coexistence** and **contestability**—not automatic decentralisation, but **rules and norms** that keep extraction **in check** until late.
